function [W,error] = omp_nnk_inverse_kernel_graph_group(G, mask, knn_param, reg, perform_OMP,maxdelay)
...
n = length(G);
ch=n./(maxdelay+1);
neighbor_indices = zeros(ch, (maxdelay+1)*knn_param);
weight_values = zeros(ch, (maxdelay+1)*knn_param);
error_values = zeros(ch,(maxdelay+1)*knn_param);
% W = sparse(n,n);
% error = sparse(n,n);
...
for i = 1:ch
    ich=[0:(maxdelay)].*ch +i;
    nodes_i=[];
    for iim=1:maxdelay+1
        nd_i = find(mask(ich(iim),:));
        nd_i(nd_i==ich(iim)) = [];
        nodes_i=[nodes_i nd_i];
    end
    %     if isempty(nodes_i)
    ...
    temp = zeros(1, (maxdelay+1)*knn_param);
...
%%
row_indices = repmat(1:ch,(maxdelay+1)*knn_param,1)';
